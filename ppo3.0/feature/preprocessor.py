...
class AStarPlanner:
    ...
    def is_action_safe(self, start_grid: Tuple[int, int], action: int, local_map: List[List[int]], half=10) -> bool:
        ...
        if 0 <= row < len(local_map) and 0 <= col < len(local_map[0]):
            return local_map[row][col] != 0  # 0 表示障碍物
        return False  # 超出局部范围视为不安全
...
class Preprocessor:
    ...
    def _get_current_target_position(self) -> Optional[Tuple[float, float]]:
        ...
        LOW_BATTERY_THRESHOLD = 0.4   # 与奖励函数一致
        FORCE_CHARGE_THRESHOLD = 0.3
    ...
    #官方无人机惩罚
        ...
        if battery_ratio < LOW_BATTERY_THRESHOLD:
            ...
            # 距离越近奖励越高，最大0.5
                charge_reward = 0.5 * (1.0 - min_charge_dist / CHARGE_REWARD_DIST)
