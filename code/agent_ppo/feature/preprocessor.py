...
class Preprocessor:
  ...
  def _reward_process(self):
    ...
    #新增奖励
    # 3. Proximity reward / 靠近目标奖励 + 到达额外奖励
    if len(self.packages) > 0 and len(self.stations) > 0:
        target_ids = set(self.packages)
        # 找到最近的驿站
        nearest_station = min(
            self.stations,
            key=lambda s: np.sqrt((s["pos"]["x"] - self.cur_pos[0]) ** 2 + (s["pos"]["z"] - self.cur_pos[1]) ** 2),
        )
        # 判断是否是目标驿站
        if nearest_station.get("config_id", 0) in target_ids:
            dist = np.sqrt(
                (nearest_station["pos"]["x"] - self.cur_pos[0]) ** 2
                + (nearest_station["pos"]["z"] - self.cur_pos[1]) ** 2
            )
            max_dist = 1.41 * 128
            # 线性靠近奖励
            reward += max(0.0, (max_dist - dist) / max_dist) * 0.5

            # 到达目标额外强化奖励
            arrival_threshold = 2.0
            if dist <= arrival_threshold:
                arrival_bonus = ((arrival_threshold - dist) / max(arrival_threshold, 1e-6)) ** 2 * 1.0
 ...
 class Preprocessor:
     ...
     def reset(self):
        ...
        # Reward hyper-parameters (可调)
        self.proximity_reward_scale = 0.5
        self.arrival_bonus_scale = 2.0
        self.arrival_threshold = 2.0
        self.obstacle_avoid_reward = 0.1
        self.stop_penalty = 0.02
        self.stop_penalty_max = 5
