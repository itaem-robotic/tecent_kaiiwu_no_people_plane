...
class Algorithm:
    ...
    def __init__(self, model, optimizer, device=None, logger=None, monitor=None):
        ...
        self.device = device
        self.logger = logger   #日志打印
        self.monitor = monitor #监控数据上报（曲线）

        self.label_size = Config.ACTION_NUM  
        self.value_num = Config.VALUE_NUM
        self.clip_param = Config.CLIP_PARAM #裁剪系数
        self.vf_coef = Config.VF_COEF       #价值损失权重
        self.var_beta = Config.BETA_START   #熵奖励权重
    ...
    def learn(self, list_sample_data):
        ...
        logits, value_pred = self.model(obs) #模型前向推理

        #计算损失并反向传播
        total_loss, info = self._compute_loss(
            logits=logits,        #动作分数（未归一化概率）
            value_pred=value_pred,#状态好坏判断
        ...
        # Periodic monitoring report / 定期上报监控 打印日志
        now = time.time()
